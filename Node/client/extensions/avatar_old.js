	...
	move(){
		...
		if(this.parent.getGame().tick % 1 == 0){
			var cmd = {
				"command" : "position",
				"id": 0,
				"_id": this.parent.player.id,
				"x": Utils.intToString(this.X),
				"y": Utils.intToString(this.Y),
				"tick": this.parent.getGame().tick
			};

			this.parent.getGame().sendPacket(cmd, this.parent);
		}
